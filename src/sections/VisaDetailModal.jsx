...
const VisaDetailModal = ({ visa, isOpen, onClose }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      gsap.fromTo(modalRef.current, { opacity: 0, scale: 0.9, y: -30 }, { opacity: 1, scale: 1, y: 0, duration: 0.4, ease: "power3.out", delay: 0.1 });
    } else {
      gsap.to(modalRef.current, { opacity: 0, scale: 0.9, y: -30, duration: 0.3, ease: "power3.in", onComplete: () => { document.body.style.overflow = "auto"; } });
  ...
  const baseButtonStyle = {
    backgroundColor: "var(--xaga-gold-dark)", color: "var(--xaga-white)",
  ...
  return (
    ...
    >
      <div
        ...
        style={{ backgroundColor: "var(--xaga-white)", maxHeight: "90vh", opacity: 0 }}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="w-full h-56 md:h-64 relative">
          <img
            src={visa.imagePath}
            alt={t('visaServices.modal.imageAlt', { name: visa.name })}
          ...
          <button
            ...
            aria-label={t('visaServices.modal.closeAriaLabel')}
          >
        ...
        <div id={`modal-description-${visa.id}`} className="p-6 md:p-8 overflow-y-auto">
          <h2
          ...
          {visa.introduction && (
            <p className="text-base text-gray-700 mb-4 leading-relaxed">{visa.introduction}</p>
          )}
          <div className="prose prose-sm md:prose-base max-w-none" style={{ color: "var(--xaga-black)" }}>
            {visa.detailedInfo && visa.detailedInfo.map((paragraph, index) => (
              <p key={index}>{paragraph}</p>
            ))}
          </div>
          <div className="mt-8 pt-6 border-t" style={{ borderColor: "var(--xaga-gold-light)" }}>
            <h4 className="text-lg font-semibold mb-3" style={{ color: "var(--xaga-gold-dark)" }}>
              {t('visaServices.modal.contactTitle')}
            </h4>
            <p className="text-sm text-gray-700 mb-4">
              {t('visaServices.modal.contactParagraph')}
            </p>
            <a
              href={`https://wa.me/${primaryWhatsAppNumber}?text=Hola,%20me%20interesa%20obtener%20más%20información%20sobre%20la%20${encodeURIComponent(visa.name)}.`}
              target="_blank"
            ...
            >
              {t('visaServices.modal.contactButton')}
            </a>
          </div>
        </div>
      </div>
